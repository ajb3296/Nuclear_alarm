...
class Help (commands.Cog) :
    ...
    async def help (self, ctx, *, arg : str  = None) :
        ...
        if arg == "GENERAL" or arg == "일반":
            ...
            embed.add_field(name=f"{commandInt}알람설정 [*켜기/끄기*]",    value="해당 채널에 알람을 설정하거나 해제합니다.", inline=False)
            embed.add_field(name=f"{commandInt}전체맨션설정 [*켜기/끄기*]", value="알람과 함께 everyone 맨션을 설정하거나 해제합니다.", inline=False)
            embed.set_footer(text=BOT_NAME_TAG_VER)
            await ctx.send(embed=embed)
            ...
            if ctx.author.id in OWNERS:
                ...
                embed.add_field(name=f"{commandInt}모듈",           value="제가 들어가 있는 모든 서버 리스트를 출력해요!", inline=False)
                embed.add_field(name=f"{commandInt}로드 [*모듈명*]",  value="제가 들어가 있는 모든 서버 리스트를 출력해요!", inline=False)
                embed.add_field(name=f"{commandInt}언로드 [*모듈명*]", value="제가 들어가 있는 모든 서버 리스트를 출력해요!", inline=False)
                embed.add_field(name=f"{commandInt}리로드 [*모듈명*]", value="제가 들어가 있는 모든 서버 리스트를 출력해요!", inline=False)
                embed.add_field(name=f"{commandInt}서버",            value="제가 들어가 있는 모든 서버 리스트를 출력해요!", inline=False)
                embed.add_field(name=f"{commandInt}방송 [*공지내용*]", value="제가 들어가 있는 모든 서버 리스트를 출력해요!", inline=False)
            ...
            embed.add_field(name=f"{commandInt}help dev",     value=">>> 원전 알람에 관한 명령어들을 보내드려요!", inline=False)
            if ctx.author.id in OWNERS:
